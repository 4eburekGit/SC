...
Иные комбинации вызывают ошибочный ответ с кодом 400 или игнорируются
...
Схема данных:
//www.plantuml.com/plantuml/png/rLJDRXCn4BxxAOPo8164Gg58e2XgBvLwgohkAxEUR0QytfNjq8GMgUsL0-f3S8yG5HLLuHMyRyIuKTAjDYlbme4ShCZVDzzyy-FTSLvQFywrUA1mI0R1otSQEvCkf5ew1nFezw7pzid3lRrT_dLXC5XIIqkvjDFa0qvZG7yycFGtntVuiB18cLdY574Q234YXP2IJJNE_-VeHuLoGiopx-SomouS2u37zxAGwi8R8ECnGyjehyV7a3JoxTFtPIO-BoJI4QKoAvJqSXsX-wWdhp4Iqf6qy96loMIThUrNtGPLeKijbPuAqmXu-UprYWXfFJcVhCwn_Nnh7b7l7SLs9AGQx1THVju4BXGJHQxKShhQTyrfYqz5EHh7k5ljAaT5uxoUfSb9BPDyeibWYutYoEX2geGTCyLpJkTbNiBHPW7-4pEqvFR8D4NdsaZBCH-YAzblN9PwsrfcN4VRProdhZ_ZPSIwCrxkRbnTLvYxeWvQjh_a--R-__zLFu93r38cSYCgdQYzNJWvwVMAOxZz8h3aHlXMdOPPk0bNrJai7WQ47-6w-WhX9qD_cF2xEWipgCvWzs1_GooMP25Mru-I5-6oEgqkM91relPDzILSBaHPx3py2jVXYiye6jdX--qvsn0yh7yZB7RGg76k_m80

Unit-тестирование

Используя библиотеку JUnit5 были написаны юнит-тесты, лежащие в server/source/test/java/museumviewer/server/
Тестами покрыто 52% кода, т.к. обмен данными с внешним API проще было тестировать интеграционно
Тесты запускаются автоматически перед запуском контейнера с сервером

Клиент тестируется интеграционно, так как представляет собой по-сути простую GUI-обёртку для запроса с сервера

Интеграционное тестирование

Используя библиотеку для unit-тестирования из фреймворка Qt - QTest, был реализован пользовательский сценарий с помощью встроенных методов эмуляции кликов мышью и ввода. Файлы самого теста находится в dockerizedClient/test/. Тест происходит не в скрытом режиме, так что можно наглядно наблюдать выполнение теста. Примечание: тест может засчитаться провалом, если по адресу, используемому в интеграционном тесте на внешнем API возникнет изображение. Для эмуляции корректной работы теста в таком случае рекомендуется отключить VPN ввиду недоступности внешнего API в РФ.

Сборка

Всё собрано в 2 контейнера docker, но ввиду некоторых ограничений запустить тот же клиент не выйдет, так как требуется разрешить доступ контейнера к дисплею, так что были созданы простенькие шелл-скрипты для:
1. Запуска сервера (сборка, тесты, запуск в фоне), базы данных и её популяции: server/server-launch.sh
2. Запуска клиента (сборка, тесты, запускать ТОЛЬКО при запущенном сервере): dockerizedClient/client-launch.sh
3. Отключения сервера (docker kill & docker rm): server/server-terminate.sh

Примечание: Рекомендуется проверить наличие флага +x у каждого из скриптов (chmod +x {скрипт} в случае необходимости)
